...
class ExchangeRatesService:
    ...
    def get_concrete_exchange_rate(self, currency_pair: str) -> ExchangeRatesDTO:
        ...
        return self.__exchange_rates_dao.get_concrete_exchange_rate(base_currency.id, target_currency.id)
    ...
    def perform_currency_exchange(self, base_currency_code: str, target_currency_code: str, amount: float):
        ...
        direct_exchange_rate = self.__exchange_rates_dao.get_concrete_exchange_rate(base_currency.id, target_currency.id)
        reverse_exchange_rate = self.__exchange_rates_dao.get_concrete_exchange_rate(base_currency.id, target_currency.id)
        ...
        if direct_exchange_rate is not None:
            ...
            usd_currency = self.__currencies_dao.get_concrete_currency('USD')
            base_usd_exchange_rate = self.__exchange_rates_dao.get_concrete_exchange_rate(base_currency.id, usd_currency.id)
            target_usd_exchange_rate = self.__exchange_rates_dao.get_concrete_exchange_rate(usd_currency.id,target_currency.id)
