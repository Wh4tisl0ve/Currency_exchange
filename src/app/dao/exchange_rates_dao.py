...
class ExchangeRatesDAO(BaseDAO):
    ...
    def get_exchange_rate(self, base_currency_id: int, target_currency_id: int) -> ExchangeRate:
        query = f'''SELECT * FROM {self._name_entity}
                    WHERE BaseCurrencyID = {base_currency_id}
                    AND TargetCurrencyID = {target_currency_id}'''
    ...
    def update(self, base_currency_id: int, target_currency_id: int, rate: float):
        query = f'''UPDATE {self._name_entity} SET Rate = {rate} 
                    WHERE BaseCurrencyId = {base_currency_id} AND TargetCurrencyId = {target_currency_id}'''
